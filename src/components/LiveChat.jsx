            ...
            <div className={` fixed z-52 md:block  flex bg-[#fcfcfc] w-full h-13 border border-solid border-gray-300 md:flex
                ...
                <div className='flex w-full md:w-3/4 items-center'>
                    <div className='flex items-center ml-10 md:ml-0'>
                        <img src={selectedUser ? `https://backend-auction-app-web.vercel.app${selectedUser.profile_photo.replace(/ /g, '%20')}` : '/default-profile.png'}
                            alt="profile photo"
                        ...
                        {contacts.map(contact => (
                            <li key={contact.id} className='mb-2'>
                                <div className='flex p-2 rounded-lg hover:bg-gray-300 text-black' onClick={() => setSelectedUser(contact)}>
                                    <div>
                                        <img src={`https://backend-auction-app-web.vercel.app${contact.profile_photo.replace(/ /g, '%20')}`}
                            ...
                            <div className='overflow-y-auto mb-50  mt-10 ' key={selectedUser.id}>
                                {messages.map((msg) => (
                                    <div key={msg.id_message} className='flex items-center'>
                                        <img src={msg.sender_profile_photo ? `https://backend-auction-app-web.vercel.app${msg.sender_profile_photo.replace(/ /g, '%20')}` : '/default-profile.png'} alt="" className={`w-[30px] h-[30px]  rounded-full mr-2 mt-5 ${msg.sender_id === sender_id ? '' : 'order-last ml-2'
                                            }`} />
                                        <div
